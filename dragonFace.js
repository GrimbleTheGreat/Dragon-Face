...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Initialize Multiplayer Connector ---
    const mpHandler = new MultiplayerHandler({
        onMove: (data) => {
            movePiece(data.startRow, data.startCol, data.move);
        },
        onPromotion: (data) => {
            performPromotion(data.row, data.col);
        },
        onReset: () => {
            resetGame();
        },
        onPlayerAssigned: (pNum) => {
            playerNumber = pNum;
            updateStatusDisplay();
        },
        onCancel: () => {
            console.log("Multiplayer cancelled.");
            playerNumber = null;
    ...
    let boardState = [];
    let currentPlayer = 1;
    let playerNumber; // Set via mpHandler callback
    ...
    function handleSquareClick(event) {
        ...
        if (selectedPiece) {
            ...
            if (move) {
                // SEND MOVE VIA MULTIPLAYER HANDLER
                mpHandler.sendMove(selectedPiece.row, selectedPiece.col, move);
    ...
    function handlePromotionClick(row, col) {
        ...
        if (isClickValid) {
            // SEND PROMOTION VIA HANDLER
            mpHandler.sendPromotion(row, col);
